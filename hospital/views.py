...
# for showing signup/login button for admin(by nelson)
...
# for showing signup/login button for doctor(by nelson)
...
# for showing signup/login button for patient(by nelson)
...
def doctor_signup_view(request):
    ...
    if request.method == 'POST':
        ...
        if userForm.is_valid() and doctorForm.is_valid():
            ...
            doctor.user = user
            doctor = doctor.save()
            # Save the doctor's profile image to HDFS
            profile_pic = request.FILES['profile_pic']  # Extract the uploaded file
...
# -----------for checking user is doctor , patient or admin(by nelson)
def is_admin(user):
    return str(user) == 'admin'


def is_doctor(user):
    return str(user) == 'doctor'


def is_patient(user):
    return str(user) == 'patient'
...
def admin_dashboard_view(request):
    ...
    hdfs_data = utils.auth_log_analysis()
    parsed_authentications = []
    for line in hdfs_data.splitlines():
        parts = line.split("\t")
        if len(parts) >= 2:
            parsed_authentications.append((parts[0], parts[1]))
    mydict = {
        'doctors': doctors,
        'patients': patients,
        ...
        'authentications': parsed_authentications,
    }
...
def admin_doctor_view(request):
    # Get all doctor records from the database
...
def contactus_view(request):
    ...
    if request.method == 'POST':
        ...
        if sub.is_valid():
            ...
            kafka_service.produce_message('test-topic', {'email': email, 'name': name, 'message': message})
